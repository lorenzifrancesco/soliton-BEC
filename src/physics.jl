...
function potential(sim::Simulation, app::Apparatus, pot::Potential, s::Float64)
    ...
    if pot.type == "barrier"
        value = pot.energy * exp(-(s - pot.position)^2 / (2 * (pot.width))^2)
...
function β_3d(sim::Simulation, app::Apparatus, pot::Potential, s::Float64)
    hbar = 6.62607015e-34 / (2 * pi)
    value = (-im / hbar) * potential(sim::Simulation, app::Apparatus, pot::Potential, s::Float64)
    return value
end
...
function get_coefficients_3d(sim::Simulation, app::Apparatus, pot::Potential, state::InitialState)
    ...
    initial_state_radial(r::Float64) = exp(-(r/(2*l_perp))^2) / (pi)^(1/2)
    confinment(r::Float64) = -1/2 * app.ω_perp^2 * app.m * r^2
