...
function ground_state_solve(sim::Simulation, coeffs::Coefficients, state::InitialState, app::Apparatus)
  pyplot()
  time_steps = Int(floor(sim.T / sim.dt))
  space_steps = Int(floor(sim.S / sim.ds))
  time = LinRange(0, sim.T, time_steps)
  ...
  #Implement external initial state 
  wave(s::Float64) = sqrt(1 / (sqrt(2 * pi) * sim.S / 3)) * exp.(-(s) .^ 2 / (4 * sim.S / 3^2)) * exp(im * s * 1.5e6)
  ...
  fwd_disp = exp.(-im * sim.dt / 2 .* coeffs.α * k .^ 2)
  fwd_curvature = exp.(-im * sim.dt / 2 .* coeffs.β.(space))
  # display("Gamma at peak")
  # display(coeffs.γ.(ψ[Int(floor(space_steps/2)),1]))
  target_Δ = 1e-10
  Δ = 1
  let prev, current
    ...
    while Δ > target_Δ
      ...
      ψ = ifft(ψ_spect)
      ψ = ψ .* exp.(im * sim.dt / 2 .* coeffs.γ.(ψ)) .* fwd_curvature  ## this is an Euler step
      # renormalization
      display(sum(abs.(ψ) .^ 2 * sim.ds))
      current = sum(abs.(ψ) .^ 2 * sim.ds)
      Δ = abs(prev - current) 
      ψ = ψ / sqrt(current)
      prev = current
    end
