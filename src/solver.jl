...
function solve(sim::Simulation, coeffs::Coefficients)
    ...
    space = LinRange(0, sim.L, space_steps)
    ...
    if (state.sech_flag == 1)
      waveform = sqrt(state.P0) .* 2 ./(exp.(space/state.S0).+exp.(-space/state.S0))
    else
      waveform = sqrt(state.P0) .* exp.(-(space/state.S0) .^ 2 ./ 2)
    end
    ...
    @showprogress "Propagating the field... " for n = 1:time_steps-1
      ...
      ψ[:, n+1] = ψ[:, n+1] .* exp.(im * sim.dt * coeffs.γ(ψ))  ## this is an Euler step
    end
